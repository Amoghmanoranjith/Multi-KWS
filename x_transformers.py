...
# qff
class QuaternionFeedForward(Module):
    def __init__(
        self,
        dim,
        dim_out = None,
        ...
        if glu:
            ...
            proj_in = nn.Sequential(
                QuaternionLinear(dim, inner_dim, bias = not no_bias),
                activation
            )

        proj_out = QuaternionLinear(inner_dim, dim_out, bias = not no_bias)
        ...
        **kwargs
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(zip(self.layer_types, shift_tokens)):
            ...
            if layer_type == 'a':
                ...
                layer = Attention(dim, heads = heads, selective = self.selective_attn, causal = causal, qkv_receive_diff_residuals = layer_qkv_receives_diff_view, learned_value_residual_mix = self_attn_learned_value_residual, rotate_num_heads = rotate_num_heads, **attn_kwargs)
                is_first_self_attn = False

            elif layer_type == 'c':
                layer = Attention(dim, heads = heads, selective = self.selective_attn, **{**attn_kwargs, **cross_attn_kwargs})
